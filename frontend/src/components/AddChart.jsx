...
export default function SensorChart({ nodo_id, dispositivo_id, sensor_id, medida_id }) {
            ...
            const fetchData = async () => {
                const allData = await Promise.all(
                    medidas.map(async ({ nodo_id, dispositivo_id, sensor_id, medida_id }) => {
                        ...
                        const responseInflux = await axios.get(`/influx/get_data?nodo_id=${nodo_id}&medida_id=${medida_id}&dispositivo_id=${dispositivo_id}&sensor_id=${sensor_id}&rango=${timeRange}`);
                        ...
                        const responseMedida = await axios.get(`/api/nodos/${nodo_id}/dispositivos/${dispositivo_id}/sensor/${sensor_id}/medidas/${medida_id}`);
    ...
    const handleAddMeasure = async (newMedida) => {
        if (typeof newMedida === "object" && newMedida !== null) {
            ...
            // Obtener unidad de medida correspondiente desde el nuevo endpoint
            const responseMedida = await axios.get(`/api/nodos/${newMedida.nodo_id}/dispositivos/${newMedida.dispositivo_id}/sensor/${newMedida.sensor_id}/medidas/${newMedida.medida_id}`);
            const unidadMedida = responseMedida.data.medida.unidad;
